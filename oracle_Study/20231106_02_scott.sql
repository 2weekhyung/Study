...
--==>>
/*
moon	문정환	010-1111-1111	제주도 서귀포시 → 데이터 변경되지 않음~!!!
wool	정한울	010-2222-2222	서울 강남구
*/
...
--○ 프로시저 생성 후 실행 테스트 (잘못된 패스워드)
EXEC PRC_STUDENTS_UPDATE('wool', 'java000', '010-8888-8888', '전남 목포');
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다.
...
/*
moon	문정환	010-9999-9999	강원도 횡성
wool	정한울	010-2222-2222	서울 강남구 → 데이터 변경되지 않음~!!!
*/

--○ 프로시저 생성 후 실행 테스트 (잘못된 패스워드)
EXEC PRC_STUDENTS_UPDATE('wool', 'java006$', '010-8888-8888', '전남 목포');
--==>> PL/SQL 프로시저가 성공적으로 완료되었습니다.
...
-- 상품코드 FK 지정
ALTER TABLE TBL_출고
ADD CONSTRAINT 출고_상품코드_FK FOREIGN KEY(상품코드) REFERENCES TBL_상품(상품코드);
--==>> Table TBL_출고이(가) 변경되었습니다.
