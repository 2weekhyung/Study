...
--○ TBL_EXAMPLE1 과 TBL_EXABPLE2 테이블이
...
※   ORACLE은 숫자 표현 타입이 한 가지로 통일되어 있다.

1. 숫자형 NUMBUR              → - 10의 38승-1 ~ 10의 38승
          NUMBER(3)           → -999 ~ 999
          NUMBER(4)           → -9999 ~ 9999
...
2. 문자형 CHAR             → 고정형 크기
          ...
          VARCHAR2(10)        ←← '강의장'        6Byte 이지만 6Byte 를 소모  
          VARCHAR2(10)        ←← '잠든김동민'    10Byte
          VRACHAR2(10)        ←← '오잠깬김동민'  10Byte 를 초과하므로 입력 불가    
...
UPDATE EMP2 SET EMPNO = 7839 WHERE EMPNO = 7389;
...
--○ 컬럼(COLUMN) 레벨의 커멘트 데이터 확인(TBL_EMP 테이블에 소속된 컬럼들만 조회)
SELECT *
FROM USER_COL_COMMENTS
...
--○ TBL_EMP 테이블에서 20번 붓에 근무하지 않는 직원들의
...
SELECT *
FROM TAB;
--==>> BIN$Y0c5rU/XRLOVtI0smDQP6Q==$0	TABLE	~
