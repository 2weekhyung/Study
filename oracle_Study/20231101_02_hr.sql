...
--○ EMPLOYESS 테이블의 직원들 SALARY를 10% 인상한다.
...
--   또한, 변경에 대한 결과 확인후 ROLLBACK 수행한다~!!!
...
UPDATE EMPLOYEES
SET SALARY = (SELECT CASE WHEN DEPARTMENT_ID = 108 THEN SALARY*1.1
                         WHEN DEPARTMENT_ID = 100 THEN SALARY*1.15
                         WHEN DEPARTMENT_ID = 205 THEN SALARY*1.2
                    END
              FROM EMPLOYEES)                 
WHERE DEPARTMENT_ID = (SELECT DEPARTMENT_ID FROM DEPARTMENTS WHERE DEPARTMENT_NAME = 'Finance')
OR DEPARTMENT_ID = (SELECT DEPARTMENT_ID FROM DEPARTMENTS WHERE DEPARTMENT_NAME = 'Executive')
OR DEPARTMENT_ID = (SELECT DEPARTMENT_ID FROM DEPARTMENTS WHERE DEPARTMENT_NAME = 'Accounting');
...
UPDATE EMPLOYEES
SET SALARY = CASE DEPARTMENT_ID WHEN (SELECT DEPARTMENT_ID FROM DEPARTMENTS WHERE DEPARTMENT_NAME = 'Finance') THEN SALARY*1.1 
                                WHEN (SELECT DEPARTMENT_ID FROM DEPARTMENTS WHERE DEPARTMENT_NAME = 'Executive') THEN SALARY*1.15
                                WHEN (SELECT DEPARTMENT_ID FROM DEPARTMENTS WHERE DEPARTMENT_NAME = 'Accounting') THEN SALARY*1.2
                                ELSE SALARY
             END
WHERE DEPARTMENT_ID IN (SELECT DEPARTMENT_ID
                        FROM DEPARTMENTS
