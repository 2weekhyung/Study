...
--○ TBL_INSA 테이블을 대상으로 신규 데이터 입력 프로시저 작성한다.
-- NUM, NAME, SSN, IBSADTE, CITY, TEL, BUSEO, JIKWI, BASICPAY, SUDANG 으로 구성된 컬럼 중
...
/*
실행 예)
EXEC PRC_INSA_INSERT('최혜인', '970812-2234567', SYSDATE, '서울', '010-2509-1783', '개발부', '대리', 2000000, 200000);
→ 위와 같이 프로시저 호출 및 실행

1061 최혜인 970812-2234567 2023-11-06 서울 010-2509-1783 개발부 대리 2000000 200000
*/
...
BEGIN
    ...
    INSERT INTO TBL_INSA(NUM, NAME, SSN, IBSADTE, CITY, TEL, BUSEO, JIKWI, BASICPAY, SUDANG)
    VALUES(V_NUM, V_NAME, V_SSN, V_IBSADTE, V_CITY, V_TEL, V_BUSEO, V_JIKWI, V_BASICPAY, V_SUDANG);
...
IS
    V_NUM TBL_INSA.NUM%TYPE;
BEGIN
    SELECT NVL(MAX(NUM),0)+1 INTO V_NUM FROM TBL_INSA;
...
--  TBL_상품 테이블의 해당 상품에 대한 재고수량이 함께 변동 될 수 있는 기능을 가진
--  프로시저를 작성한다.
...
IS
    ...
    USER_DEFINE_ERROR   EXCEPTION;
BEGIN
    SELECT NVL(MAX(출고번호),0) +1 INTO V_출고번호 FROM TBL_출고;
    
    SELECT NVL(재고수량,0) INTO V_재고수량 FROM TBL_상품 WHERE 상품코드 = V_상품코드;
...
BEGIN
    ...
    SELECT 재고수량 INTO V_현재재고 FROM TBL_상품 WHERE 상품코드 = V_상품코드;
    
    SELECT 출고수량 INTO V_변경전출고 FROM TBL_출고 WHERE 출고번호 = V_출고번호;
    
    IF (V_변경할수량 - V_변경전출고 > V_현재재고)
        THEN RAISE USER_DEFINE_ERROR;
    END IF;
