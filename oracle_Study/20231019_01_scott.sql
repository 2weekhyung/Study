...
SLECT 사원번호, 사원명, 급여, 커미션, 연봉
FROM TBL_EMP;

SELECT EMPNO "사원번호", ENAME "사원명", SAL "급여", COMM "커미션"--, 100 "연봉"
FROM TBL_EMP;
...
SELECT EMPNO "사원번호", ENAME "사원명", SAL "급여", COMM "커미션", NVL2((SAL*12+COMM),(SAL*12+COMM),SAL*12) "연봉"
FROM TBL_EMP;
--==>>
/*
7369	SMITH	800		        9600
...
-- 맨 앞에 있는 매개변수 부터 차례로 NULL 인지 아닌지 확인하여
-- NULL 아 아닐 경우 그 값을 반환하고,
...
SELECT EMPNO "사원번호", ENAME "사원명", SAL"급여", COMM"커미션", COALESCE(SAL*12+COMM,SAL*12,COMM)"연봉"
FROM TBL_EMP;
...
FROM DUAL;
--==>> 지민이는	2023-10-19 10:41:05	에 연봉	    500	      억을 원한다.
--     -------- ------------------- -------     ---       ------------
...
FROM DUAL;
--==>> 2023-10-19 10:43:16	2023-10-19 10:43:16	23/10/19 10:43:16.000000000

SELECT '지민이는'|| SYSDATE || '에 연봉' || 500 || '억을 원한다.'
FROM DUAL;
--==>> 지민이는2023-10-19 10:44:45에 연봉500억을 원한다.
...
--   다시 마지막에 등장하는 홑따옴표 하나(『'』)는 문자열 영역의 종료를 의미하게 되는 것 이다.
...
--○ TBL_EMP 테이블에서 입사일이 1981 9월 28일 이후
...
--○ TBL_EMP 테이블에서 입사일이 1981 4월 2일 부터
...
VALUES(1001, '강혜성','9710171234567',TO_DATE('2005-01-31', 'YYYY-MM-DD'), 3000);
...
--○ TBL_SAWON 테이블에서 사원이 이름이 『영』으로 끝나는 사원의
--   사원명, 주민번호, 급여 항목을 조회한다.
SELECT SANAME "사원명", JUBUN "주민번호", SAL "급여"
FROM TBL_SAWON
WHERE SANAME LIKE '%영';
...
--○ TBL_SAWON 테이블에서 사원이 이름에 '이'라는 글자가
--   하나라도 포함되어 있다면 그 사원의
...
40	OPERATIONS	BOSTON
*/
--> 50번 개발부 서울... 에 대한 데이터가 소실되었음을 확인(존재하는지 않음)
...
--※ COMMIT 을 실행한 이후로 DML(INSERT, UPDATE, DELETE, 등 데이터 조작 언어) 구문을 통해
--   변경된 데이터를 취소할 수 있는 것일 뿐...
--   DML 명령을 사용한 후 COMMIT 을 수행하고나서 ROLLBACK 을 실행해봐야
--   아무 소용이 없다.
