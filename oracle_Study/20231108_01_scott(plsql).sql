...
BEGIN
    ...
    IF (INSERTING)
        THEN IF (V_재고수량 - :NEW.출고수량 < 0) THEN RAISE USER_DEFINE_ERROR; END IF; 
            UPDATE TBL_상품
            SET 재고수량 = 재고수량 - :NEW.출고수량
            WHERE 상품코드 = :NEW.상품코드;
            
    ELSIF (UPDATING)
        THEN IF (V_재고수량 + :OLD.출고수량 - :NEW.출고수량 < 0) THEN RAISE USER_DEFINE_ERROR; END IF;
            UPDATE TBL_상품
