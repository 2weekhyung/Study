...
-- 컬럼명 데이터타입,
-- 컬럼명 데이터타입,
-- CONSTRAINT CONSTRAINT명 CHEK(컬럼 조건)
...
--   데이터 입력이나 수정시, 성별이 유효한 데이터만 입력될수 있도록
...
ADD ( CONSTRAINT TESTMEMBER_SID_PK PRIMARY KEY(SID)
    , CONSTRAINT TESTMEMBER_SSN_CK CHECK(SUBSTR(SSN 8번째 자리 1개가) '1' 또는 '2' 또는 '3' 또는 '4'));
...
-- 2. 부모 테이블(참조받는 컬럼이 포함된 테이블)이 먼전 생성된 후
--    자식 테이블(참조하는 컬럼이 포함된 테이블)이 먼저 생성되어야 한다.
--    이 때, 자식 테이블에 FOREIGN KEY 제약조건이 설정된다.

-- 3. 형식 및 구조
-- ① 컬럼 레벨의 형식
-- 컬럼명 데이터 타입 [CONSTRAINT CONSTRAINT 명]
...
비워 두는건 가능하지만, 입력을 한다면 올바른 값을 입력해야함
*/
...
--    또한, 부모 테이블을 제거하기 위해서는 자식 테이블을 먼저 제거해햐 한다.
...
-- 2. 형식 및 구조
-- ① 컬럼 레벨의 형식
-- 컬럼명 데이터타입 [CONSTRAINT CONSTRAINT명] NOT NULL      -- 다른 제약조건과 달리 컬럼 레벨이 기본
...
-- ALTER TABLE 테이블명
-- MODIFY 컬렴명 데이터타입 NOT NULL

-- 4. 기존 테이블에 데이터가 이미 들어있지 않은 컬럼(→ NULL)을
...
--==>>
/*
HR	SYS_C007044	TBL_TEST11	C	COL2	"COL2" IS NOT NULL	
HR	SYS_C007045	TBL_TEST11	P	COL1		
*/
...
HR	TEST13_COL1_PK	TBL_TEST13	P	COL1		
HR	TEST13_COL2_NN	TBL_TEST13	C	COL2	COL2 IS NOT NULL	
HR	SYS_C007050	TBL_TEST13	C	COL2	"COL2" IS NOT NULL	
*/
...
2	김다슬	오라클 DEFAULT 표현식을 실습중입니다.	2023-10-31 14:45:09	0	0
3	노은하	계속 실습중입니다.	2023-10-31 14:46:16	0	0
4	문정환	열심히 실습중입니다.	2023-10-31 14:47:11	0	0
5	이윤수	무진장 실습중입니다.	2023-10-31 14:48:06	0	0
*/
