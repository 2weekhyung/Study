...
○ Wrapper 클래스
	...
	2. 기본 자료형인
	byte, short, int,		long, float, double, char		bolean 형에 대응하는
...
※ 오토 박싱(Auto-Boxing)과 오토 언박싱(Auto-Unboxing)
   ...
   이 경우 발생하는 문제 해결을 이해 자바에서는 Wrapper 클래스를 제공하게 되었다.
   ...
   // 기본 자료형을 객체 타입으로 바꾸니까 박싱

   b = ob;
   //-- Integer(객체)가 기본 자료형 int 형으로 자동 변환(오토 언박싱)
   // 실제로는... 『b = ob.intValue();』 //(언박싱)

 //  객체 타입을 기본 자료형으로 바꾸니까 언박싱
...
public class Test130
{
	...
	{	
		...
		d = new Integer(10);	//-- 업 캐스팅 생성된 건 하위 객체인데 상위 객체에 대입하려면
		System.out.println("d.toString : " + d.toString());
		//--==>> d.toString : 10
		//-- Object 클래스가 시조 클래스라 자손인 Integer 에도 toString 메소드가 있다 여기서 오버라이딩 된 결과

		d = new Double(12.345);	//-- 업 캐스팅 생성된 건 하위 객체인데 상위 객체에 대입하려면 
		System.out.println("d.toString : " + d.toString());
		//--==>> d.toString : 12.345
