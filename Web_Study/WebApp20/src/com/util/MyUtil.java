...
public class MyUtil
{
	...
	public String pageIndexList(int currentPage, int totalPage, String listUrl)
	{
		...
		//   1페이지도 못채우는 경우는 별도의 페이징 처리를 할 필요가 없다.
		if (currentPage==0)
		{
			return "";
		}
		...
		*/
		...
		if (listUrl.indexOf("?") != -1)	// 링크를 설정할 URL 에 『?』가 들어왔으면...
		{
		...
		//-- 예를 들어, 검색값이 존재하면
		//   이미 request 값에 seqrchKey 와 searchValue 가 들어있는 상황이므로
		...
		//-- listUrl 은 위에서 (라인 78 ~ 85) 이미 전처리가 끝난 상황이기 때문에
		//   『...?』 상태 또는 『...?...&』 인 상태이다.
		//   이로 인해 결과는
