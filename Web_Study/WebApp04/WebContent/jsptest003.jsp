...
<!-- 
	※ 요청한 페이지(jsptest003.jsp)의 결과를 확인하 후 새로고침을 반복해보면...
	a 변수(선언부에 선언한 변수) 값만 계쏙 증가하는 것을 확인할 수 있다.
	a 변수 값만 계속 증가하는 이유를 확인하기 위해서는
	서블릿(servlet) 코드를 분석해 보야야 한다.
	...
	- 경로
	  ...
	  ==> 변수 a 는 클래스(jsptest003.jsp)의 전역변수로 선언된 것을 확인할 수 있다.
	      이는 객체가 소멸되기 전까지는 계속 값이 유지된다는 것을 의마한다.
	      변수 b는 메소드(jsptest003_jsp 클래스의 _jspService() 메소드) 내의
